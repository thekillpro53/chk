...
from .func.shf_defs import *
from .func.rand_user import random_user_api, PROXIES


@register(cmds="shf", is_text=True)
@gate_info_dec('shf')
@get_strings_dec("card_check")
@get_spam_dec()
async def shf(message, gate_info, user_info,start_time, lang):
    try:
        ...
        r_text, r_logo, r_respo = get_response_shf(last)
        if 'Charged $64' in r_text:
