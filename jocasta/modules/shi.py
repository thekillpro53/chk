...
from .func.shi_defs import *
from .func.rand_user import random_user_api, PROXIES


@register(cmds="shi",  is_text=True)
@gate_info_dec('shi')
@get_strings_dec("card_check")
@get_spam_dec()
async def shi(message, gate_info, user_info,start_time, lang):
    try:
        ...
        varient_id = one(browser)
        assert varient_id, lang['first_error']
        assert two(browser), lang['second_error']
        ...
        auth_token, url = c
        payment_gateway = four(browser,url,auth_token, rand_user)
        assert payment_gateway, lang['fourth_error']
        await msg.edit_text(lang['half_half_msg'].format(card=lista, name=message.from_user.first_name, id=message.from_user.id,
                                    bin_bank=bin_info['bank_name'], gate_name=gate_info['name'],
                                    elapsed=round(time.time() - start_time), bin_country=bin_info['country'],
                                    flag=bin_info['flag'], vendor=bin_info['vendor'], level=bin_info['level'],
                                    type=bin_info['type'], role=user_info['role']), disable_web_page_preview=True)
        fiv =five(browser, payment_gateway, data, rand_user,auth_token, url)
        assert fiv, lang['fifth_error']
        r_text, r_logo, r_respo = get_response_shi(fiv)
        if 'Charged $4' in r_text:
