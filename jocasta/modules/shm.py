...
from .func.shm_defs import *
from .func.rand_user import random_user_api, PROXIES



@register(cmds="shm", is_text=True)
@gate_info_dec('shm')
@get_strings_dec("card_check")
@get_spam_dec()
async def shm(message, gate_info, user_info,start_time, lang):
    try:
        ...
        r_text, r_logo, r_respo = get_response_shm(last)
        if 'Charged $9' in r_text:
